...
### 5. 504 Gateway Timeout при получении остатков вариантов
**Проблема**: Отдельные запросы `/report/stock/bystore` для каждого варианта вызывали таймауты

**Решение**:
- Добавлен параметр `expand=stock` в запрос вариантов
- Убраны отдельные запросы остатков, данные берутся напрямую из варианта

### 6. TypeScript warning - unused parameter
