...
interface BasketDepthChartProps {
  data: DataPoint[];
...
export default function BasketDepthChart({ data, interval, chartType = 'area', isLoading }: BasketDepthChartProps) {
  if (isLoading) {
    return (
  ...
  const chartData = data.map(point => ({
    ...
    depth: point.averageBasketDepth,
  }));
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <motion.div
          ...
          className="bg-tg-secondary-bg border-2 border-indigo-500/30 rounded-xl p-3 shadow-lg"
        >
          <p className="text-sm font-semibold text-tg-text mb-1">{datum.name}</p>
          <p className="text-lg font-bold text-indigo-400">{formatNumber(datum.depth)}</p>
          <p className="text-xs text-tg-hint mt-1">товаров в среднем</p>
        </motion.div>
      );
    }
    return null;
    ...
    <ResponsiveContainer width="100%" height={300}>
      {chartType === 'area' ? (
        <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="basketDepthGradient" x1="0" y1="0" x2="0" y2="1">
