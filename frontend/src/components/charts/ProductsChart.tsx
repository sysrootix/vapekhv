...
export default function ProductsChart({ data, interval, chartType = 'area', isLoading }: ProductsChartProps) {
  if (isLoading) {
    return (
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <motion.div
          ...
          className="bg-tg-secondary-bg border-2 border-orange-500/30 rounded-xl p-3 shadow-lg"
        >
          <p className="text-sm font-semibold text-tg-text mb-1">{datum.name}</p>
          <p className="text-lg font-bold text-orange-400">{formatNumber(datum.products)}</p>
          <p className="text-xs text-tg-hint mt-1">товаров</p>
        </motion.div>
      );
    }
    return null;
