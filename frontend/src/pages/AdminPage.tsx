...
const intervalOptions = ['daily', 'weekly', 'monthly'] as const;
const crmPeriodOptions: Record<(typeof intervalOptions)[number], number[]> = {
  daily: [7, 14, 30],
  weekly: [6, 12, 24],
  monthly: [6, 12, 18],
};

const newUsersPeriodOptions: Record<(typeof intervalOptions)[number], number[]> = {
  daily: [7, 14, 30],
  weekly: [6, 12, 24],
  monthly: [6, 12, 18],
};
...
export default function AdminPage() {
  ...
  const showOrders = Boolean(access?.permissions.manageOrders);
  const showCrm = Boolean(access?.permissions.viewCrm);
  ...
  }, [crmUserDetails?.user.firstName, crmUserDetails?.user.lastName, crmUserDetails?.user.username, crmUserDetails?.user.phone, crmUserDetails?.user.bonusPoints]);
      ...
      <div className="min-h-screen flex items-center justify-center bg-tg-bg">
        <div className="max-w-md mx-auto text-center space-y-4 p-8 bg-tg-secondary-bg rounded-3xl">
          ...
          <p className="text-sm text-tg-hint">
            Попросите администратора добавить ваш Telegram ID в список CRM или администраторов.
          </p>
        </div>
      </div>
    );
  ...
  const headerTitle = showOrders && showCrm
    ? 'Рабочая панель'
    : showOrders
      ? 'Админ-панель'
      : 'CRM-аналитика';

  const headerSubtitle = showOrders && showCrm
    ? 'Заказы и CRM-аналитика теперь в одном месте'
    : showOrders
      ? 'Управляйте заказами и статусами доставки'
      : 'Следите за выручкой и клиентами';
    ...
    <div className="min-h-screen bg-tg-bg pb-6">
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        >
          ...
          <div>
            <h1 className="text-2xl font-bold text-tg-text">{headerTitle}</h1>
            <p className="text-sm text-tg-hint">{headerSubtitle}</p>
          </div>
        </motion.div>
