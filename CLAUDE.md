...
3. **НЕ ЗАПУСКАЙ npm install** - установка зависимостей выполняется на сервере, не на MacBook. Если нужны новые пакеты - просто добавь их в package.json, а установку сделают на сервере.
...
- **НИКОГДА не запускай `npm install`** - установка зависимостей делается на сервере
...
2. ❌ **НЕ запускай `npm install`** - зависимости ставятся на сервере
